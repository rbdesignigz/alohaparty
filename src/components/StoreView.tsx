      ...
      <aside id="store-sidebar" className="w-full lg:w-64 shrink-0 flex flex-col gap-6">
        ...
        <div id="filter-categories-card" className="bg-[#ffffff] rounded-2xl p-6 border border-[#867273]/20 shadow-sm">
          <h3 className="font-sans font-bold text-base text-[#1b1c1c] mb-4 flex items-center gap-2 border-b border-[#867273]/10 pb-2">
            <Filter className="w-4 h-4 text-[#f4949a]" />
            Categorías
          </h3>
          <div className="flex flex-col gap-3">
            {categories.map((cat) => {
              ...
              return (
                <label 
                  key={cat} 
                  className="flex items-center gap-3 text-xs text-[#534343] cursor-pointer hover:text-[#f4949a] transition-colors"
                >
                  <input
                    type="checkbox"
                    checked={isChecked}
                  ...
                  />
                  <span className={`${isChecked ? 'font-semibold text-[#f4949a]' : ''}`}>
                    {cat}
                  </span>
                </label>
              );
        ...
        <div id="filter-price-card" className="bg-[#ffffff] rounded-2xl p-6 border border-[#867273]/20 shadow-sm">
          <h3 className="font-sans font-bold text-base text-[#1b1c1c] mb-4 flex items-center gap-2 border-b border-[#867273]/10 pb-2">
            <SlidersHorizontal className="w-4 h-4 text-[#f4949a]" />
            Precio Máximo
          </h3>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex justify-between text-xs font-semibold text-[#867273] mt-2">
              <span>$0</span>
              <span className="text-[#f4949a] bg-[#f4949a]/10 px-2.5 py-0.5 rounded-full">${maxPrice} ARS</span>
              <span>$100,000+</span>
            </div>
          </div>
        </div>
      ...
      <main id="store-catalog-main" className="flex-grow flex flex-col gap-6">
          ...
          <div id="sort-tabs-container" className="flex gap-2 bg-[#efeded]/80 p-1 rounded-full self-stretch sm:self-auto justify-center">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                sortBy === 'recientes'
                  ? 'bg-white text-[#f4949a] shadow-sm'
        ...
        {paginatedProducts.length === 0 ? (
          <div id="catalog-empty-state" className="flex flex-col items-center justify-center py-16 px-4 bg-white rounded-3xl border border-[#867273]/20 shadow-sm text-center">
            <Sparkles className="w-12 h-12 text-[#f4949a]/30 mb-4 animate-bounce" />
            ...
            <button
              ...
              className="mt-4 px-4 py-1.5 bg-[#f4949a] text-white text-xs font-semibold rounded-full hover:bg-[#e3858b] transition-all cursor-pointer"
            >
              Restablecer Filtros
            </button>
          </div>
              ...
              return (
                ...
                >
                  ...
                  {product.isPopular && !product.isRecent && (
                    <span className="absolute top-3 left-3 bg-[#ffb3b6]/60 text-[#e3858b] text-[9px] font-bold px-2 py-0.5 rounded-full z-10 shadow-sm flex items-center gap-1">
                      <Star className="w-2.5 h-2.5 fill-[#e3858b]" />
                      Destacado
                    </span>
                  )}
                  ...
                  <div className="p-5 flex flex-col flex-grow justify-between gap-4">
                    <div className="text-left flex flex-col gap-1.5">
                      <h3 className="font-sans font-bold text-sm text-[#1b1c1c] leading-tight group-hover:text-[#f4949a] transition-colors">
                        {product.name}
                      </h3>
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      ...
                      {showTextButton ? (
                        <button
                          ...
                          className="px-5 py-2 bg-[#f4949a] text-white text-[11px] font-bold rounded-full hover:bg-[#e3858b] shadow-xs hover:-translate-y-0.5 active:translate-y-0 active:scale-95 transition-all duration-150 flex items-center gap-1 cursor-pointer border border-[#867273]/20"
                        >
                        ...
                        <button
                          ...
                          onClick={() => addToCart(product)}
                          className="w-9 h-9 bg-[#f4949a] hover:bg-[#e3858b] text-white rounded-full flex items-center justify-center shadow-xs hover:-translate-y-0.5 active:translate-y-0 active:scale-95 transition-all duration-150 cursor-pointer border border-[#867273]/10"
